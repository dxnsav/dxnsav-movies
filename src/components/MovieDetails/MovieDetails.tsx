...
const MovieDetails = () => {
	...
	const areInWatchlist = useWatchListStore((state) => state.areInWatchList);
	...
	useEffect(() => {
		// If the movie is not found, navigate to the main page
		if (!movie) {
			navigate("/");
		} else {
			console.log(movie);
	...
	useEffect(() => {
		if (movie) fetchSimilarMovies(movie.id);
	...
	const onStateChange = (movieId) => {
		if (watchList.has(movieId)) removeFromWatchList(movieId, userId);
		else addToWatchList(movieId, userId);
	};
					...
					>
						...
						<div className="absolute w-full mx-auto bottom-[10%] px-4 z-50 portrait:hidden">
							<div className="w-full flex flex-col justify-center">
								<h1 className="text-[2em] font-unbounded w-[60%]">
									{movie.title}
								</h1>
							</div>
